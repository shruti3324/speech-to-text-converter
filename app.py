...
class MicAudioProcessor(AudioProcessorBase):
    ...
    def recv(self, frame: av.AudioFrame) -> av.AudioFrame:
        ...
        print("Got audio frame:", frame.shape)
        if frame.ndim > 1:
                ...
                if self.speech_started:
                    ...
                    if self.silence_ms >= self.max_silence_after_speech_ms:
                        ...
                        try:
                            ...
                            print(">>> Sent chunk to ASR queue:", len(chunk_pcm))
                        except queue.Full:
                            _ = chunk_queue.get_nowait()  # drop oldest
                            chunk_queue.put_nowait(chunk_pcm)
                            print(">>> Queue full, replaced oldest, new chunk size:", len(chunk_pcm))
                        self.speech_started = False
                        self.silence_ms = 0

        return av.AudioFrame.from_ndarray(frame, layout="mono")
...
def asr_worker(stop_event: threading.Event, waveform_placeholder):
    ...
    while not stop_event.is_set():
        try:
            ...
            print("<<< Got chunk from queue:", len(pcm_chunk))
        except queue.Empty:
            continue
...
with tab2:
    ...
    ctx = webrtc_streamer(
        ...
        audio_processor_factory=MicAudioProcessor,   # always register
        async_processing=True,
    )


    if start and ctx.state.playing and not st.session_state.mic_active:
        ...
        worker = threading.Thread(
            target=asr_worker, args=(stop_event, waveform_placeholder), daemon=True
        )
        worker.start()
